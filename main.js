...
function onloadEvent() {
	...
	EXIF.getData(img, function() {
		...
		exifstr += model ? `${model}  ` : ""
		const focalLength = +data.FocalLength ? `${data.FocalLength}mm` : data.FocalLength === undefined ? "" : "––mm"
		exifstr += focalLength ? `${focalLength}  ` : ""
		const FNumber = +data.FNumber ? `F${data.FNumber}` : data.FNumber === undefined ? "" : "F––"
